...
#
# with open('v_100_100_10_2d_03cfl.pickle', 'wb') as f:
#     pickle.dump(out, f)

# with open('v_100_100_15_2d_03cfl.pickle', 'rb') as f:
#     out = pickle.load(f)
with open('fix.pickle', 'rb') as f:
    out = pickle.load(f)
u = out['u']
v = out['v']
space_x = out['space_x']
...
def update(num, my_ax, v_):
    my_ax.clear()

    Z_ = v_[num][:, :, 0]
    Z_1d = Z_.ravel()
    my_ax.scatter(space_x_1d, space_y_1d, Z_1d)
    ax.set_zlim(-0.1, 3)


ani = animation.FuncAnimation(fig, update, N, fargs=(ax, v), interval=1, blit=False)

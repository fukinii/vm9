...
class Euler:
    ...
    def estimate_pressure_based_speed_of_wave(self, ul, ur):
        ...
        aver_a = (al + ar) / 2
        p_pvrs = (vl[2] + vr[2] - (vr[1] - vl[1]) * aver_rho * aver_a) / 2
        p_star = max(0, p_pvrs)
        ql = self.calc_qk(vl[2], p_star)
        qr = self.calc_qk(vr[2], p_star)
    ...
    def calc_s_star(vl, vr, sr, sl):
        ...
        s_star = (vr[2] - vl[2] + vl[0] * vl[1] * (sl - vl[1]) - vr[0] * vr[1] * (sr - vr[1])) / (
    ...
    def calc_uk_star(uk, vk, s_star, sk):
        ...
        uk_star[0] = 1
        uk_star[1] = s_star
        uk_star[2] = (uk[2] / vk[0] + (s_star - vk[1]) * (s_star + vk[2] / vk[0] / (sk - vk[1])))
    ...
    def flux_riemann_solver(self, ul, ur):
        if self.riemann_solver == 'hllc':
            return self.calc_flux_hllc(ul, ur)

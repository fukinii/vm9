...
'''Тест 3'''
# w_left = np.array([1., 0.0, 1000.])
# w_right = np.array([2., 0.0, 0.01])
# x0 = 0.5
# t_final = 0.012

'''Тест 2'''
w_left = np.array([1., -2, 0.4])
w_right = np.array([1, 2.0, 0.4])
x0 = 0.5
t_final = 0.15
...
euler_model = Euler(initial_conditions_=[w_left, w_right, x0], wave_speed_estimator_='pressure_based', dim=1)
# solver = SolverEno(model=euler_model, cfl=0.15)
simple_solver = SimpleSolver(model=euler_model, cfl=0.15)

u, space, t_final = simple_solver.solve_problem(position_nodes_number=position_nodes_number,
                                                t_final=t_final)
